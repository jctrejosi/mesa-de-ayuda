...
import {
  MapPin, CheckCircle2, AlertTriangle, Clock, LogIn, LogOut,
  Info, User, History, Home, X, Signal, ChevronRight, Shield,
  Eye, EyeOff, Mail, Lock, Loader2, Building2, LayoutGrid,
  ChevronDown, Star,
} from "lucide-react";
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background: "linear-gradient(145deg, #1E3A8A 0%, #2563EB 45%, #3B82F6 75%, #60A5FA 100%)",
        }}
      >
        ...
        <div className="absolute inset-0">
          ...
          <div className="absolute inset-0" style={{
            background: "linear-gradient(145deg, rgba(30,58,138,0.95) 0%, rgba(37,99,235,0.85) 50%, rgba(59,130,246,0.80) 100%)"
          }} />
        </div>
        ...
        <div className="relative z-10 flex flex-col h-full p-10">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-white font-bold text-lg leading-none">NexusERP</p>
              <p className="text-blue-200 text-[11px] font-medium mt-0.5">Enterprise Suite</p>
            </div>
          </div>
          ...
          <div className="flex-1 flex flex-col justify-center">
            ...
            <div className="flex flex-wrap gap-2 mb-8">
              {["Finanzas", "RRHH", "Logística", "CRM", "Reportes"].map((m) => (
                <span key={m} className="text-[11px] font-semibold text-blue-100 bg-white/10 border border-white/20 px-3 py-1 rounded-full backdrop-blur-sm">
                  {m}
                </span>
              ))}
            </div>
            ...
            <p className="text-blue-100 text-[15px] leading-relaxed max-w-xs">
              Plataforma integral para la gestión empresarial. Conecta tus equipos, optimiza tus procesos y toma mejores decisiones.
            </p>
            ...
            <div className="mt-10 grid grid-cols-3 gap-4">
              {[
                ...
                <div key={s.label} className="bg-white/10 border border-white/20 rounded-[14px] p-3 backdrop-blur-sm text-center">
            ...
            <div className="mt-6 bg-white/10 border border-white/20 rounded-[16px] p-4 backdrop-blur-sm">
              ...
              <p className="text-blue-50 text-[13px] leading-relaxed italic">
                "NexusERP transformó completamente la forma en que gestionamos nuestras operaciones."
              </p>
              <div className="flex items-center gap-2 mt-3">
                ...
                <div>
                  <p className="text-white text-[12px] font-semibold">María Rodríguez</p>
                  <p className="text-blue-200 text-[10px]">CTO · Grupo Empresarial Norte</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="w-full max-w-[440px]">
            ...
            <div className="bg-white rounded-[24px] shadow-[0_8px_40px_rgba(15,23,42,0.10)] p-8 lg:p-10">
              ...
              <div className="flex items-center gap-3 mb-6">
                ...
                <span className="text-[12px] text-[#94A3B8] font-medium">o continúa con</span>
                <div className="flex-1 h-px bg-[#E2E8F0]" />
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4" noValidate>
                ...
                <div>
                  ...
                  <div className={`relative flex items-center rounded-[12px] border-2 bg-[#F8FAFC] transition-all ${
                    errors.email
                      ? "border-[#EF4444] bg-[#FFF5F5]"
                      : "border-[#E2E8F0] focus-within:border-[#2563EB] focus-within:bg-white"
                  }`}>
                    <Mail size={16} className="absolute left-3.5 text-[#94A3B8]" />
                    <input
                      type="text"
                      value={email}
                ...
                <div>
                  ...
                  <div className={`relative flex items-center rounded-[12px] border-2 bg-[#F8FAFC] transition-all ${
                    ...
                    <Lock size={16} className="absolute left-3.5 text-[#94A3B8]" />
                    <input
            ...
            <div className="mt-6 text-center space-y-1.5">
              <p className="text-[12px] text-[#94A3B8]">
                ...
                <span className="font-semibold text-[#64748B]">Nexus Technologies S.A.S.</span>
              </p>
              <p className="text-[12px] text-[#94A3B8]">
                <button className="hover:text-[#2563EB] transition-colors">Política de privacidad</button>
                {" · "}
                <button className="hover:text-[#2563EB] transition-colors">Términos de uso</button>
              </p>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="mx-4 bg-white rounded-[20px] shadow-[0_2px_16px_rgba(15,23,42,0.08)] p-4 flex items-center gap-4">
      <div className="relative shrink-0">
        <img src={EMPLOYEE.avatar} alt={EMPLOYEE.name} className="w-[60px] h-[60px] rounded-full object-cover bg-[#E8ECF2]" />
        <span className="absolute -bottom-0.5 -right-0.5 w-4 h-4 rounded-full bg-[#16A34A] border-2 border-white" />
      </div>
      <div className="min-w-0">
        <p className="font-semibold text-[#0F1523] text-[15px] truncate">{EMPLOYEE.name}</p>
        <div className="flex items-center gap-2 mt-0.5">
          <span className="text-xs text-[#64748B] font-medium">{EMPLOYEE.code}</span>
          <span className="w-1 h-1 rounded-full bg-[#CBD5E1]" />
          <span className="text-xs text-[#64748B] truncate">{EMPLOYEE.role}</span>
        </div>
...
function GpsPrecisionBadge({ precision }: { precision: GpsPrecision }) {
  const map = {
    excellent: { label: "GPS Excelente", color: "text-[#16A34A]", bg: "bg-[#F0FDF4]" },
  ...
  return (
    <span className={`inline-flex items-center gap-1 text-[11px] font-semibold px-2 py-0.5 rounded-full ${p.bg} ${p.color}`}>
      <Signal size={10} />
      {p.label}
    </span>
  );
...
function LocationCard({ screen, precision }: { screen: AppScreen; precision: GpsPrecision }) {
      ...
      {isLoading ? (
        <div className="flex flex-col items-center py-3 gap-3">
          ...
          <div className="text-center">
            <p className="font-semibold text-[#0F1523] text-[15px]">Obteniendo ubicación…</p>
            <p className="text-xs text-[#64748B] mt-0.5">Verificando señal GPS</p>
          </div>
        </div>
      ) : (
        <div className="flex items-start gap-4">
          <div className={`w-14 h-14 shrink-0 rounded-[16px] flex items-center justify-center ${isValid ? "bg-[#F0FDF4]" : "bg-[#FEF2F2]"}`}>
            <MapPin size={26} className={isValid ? "text-[#16A34A]" : "text-[#DC2626]"} strokeWidth={2} />
          </div>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              <p className={`font-semibold text-[15px] ${isValid ? "text-[#15803D]" : "text-[#B91C1C]"}`}>
                {isValid ? "Ubicación verificada" : "Fuera del área"}
              </p>
              {isValid ? <CheckCircle2 size={16} className="text-[#16A34A]" /> : <AlertTriangle size={16} className="text-[#DC2626]" />}
            </div>
            ...
            <div className="mt-2.5"><GpsPrecisionBadge precision={precision} /></div>
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <div className="mx-4 bg-white rounded-[20px] shadow-[0_2px_16px_rgba(15,23,42,0.08)] px-5 py-4">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-xs font-medium text-[#64748B] capitalize">{formatDate(now)}</span>
      </div>
...
function MainButton({ screen, attendanceType, onPress }: { screen: AppScreen; attendanceType: AttendanceType; onPress: () => void }) {
  ...
  return (
    <div className="mx-4">
      ...
      >
        {enabled ? (
          <>{isEntrada ? <LogIn size={20} strokeWidth={2} /> : <LogOut size={20} strokeWidth={2} />}
          {isEntrada ? "Registrar Entrada" : "Registrar Salida"}</>
        ) : screen === "loading" ? (
          <><div className="w-5 h-5 rounded-full border-2 border-[#94A3B8] border-t-transparent animate-spin" />Obteniendo ubicación…</>
        ) : (
          <><MapPin size={20} />No disponible</>
        )}
      </button>
    </div>
  );
  ...
  return (
    <div className="mx-4 bg-[#EFF3FF] rounded-[16px] p-4 flex items-start gap-3">
      ...
      <p className="text-xs text-[#3B5EA6] leading-relaxed">
        El registro únicamente puede realizarse dentro del área autorizada. La ubicación se verifica automáticamente.
      </p>
    </div>
  );
}
...
function BottomNav({ active, onSelect }: { active: NavTab; onSelect: (t: NavTab) => void }) {
  const tabs: { id: NavTab; label: string; icon: React.ReactNode }[] = [
    { id: "asistencia", label: "Asistencia", icon: <Home size={22} strokeWidth={1.8} /> },
    { id: "historial", label: "Historial", icon: <History size={22} strokeWidth={1.8} /> },
    ...
    <div className="bg-white border-t border-black/[0.06] flex pb-8 pt-2 px-4">
      {tabs.map((tab) => (
        <button key={tab.id} onClick={() => onSelect(tab.id)}
          ...
          {active === tab.id && <span className="w-1 h-1 rounded-full bg-[#2563EB]" />}
        </button>
      ))}
    </div>
  );
...
function ConfirmationModal({ type, time, onClose }: { type: AttendanceType; time: string; onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center">
      ...
      <div className="relative w-full max-w-[430px] bg-white rounded-t-[28px] p-6 pb-10 animate-[slideUp_0.35s_cubic-bezier(0.34,1.56,0.64,1)_both]">
        <button onClick={onClose} className="absolute top-4 right-4 w-8 h-8 rounded-full bg-[#F1F5F9] flex items-center justify-center text-[#64748B]">
          <X size={16} />
        </button>
        <div className="flex flex-col items-center text-center pt-2">
          <div className="w-20 h-20 rounded-full bg-[#F0FDF4] flex items-center justify-center mb-4 shadow-[0_0_0_8px_#DCFCE7]">
            <CheckCircle2 size={40} className="text-[#16A34A]" strokeWidth={1.8} />
          </div>
          <span className="inline-flex items-center gap-1.5 bg-[#F0FDF4] text-[#15803D] text-xs font-semibold px-3 py-1 rounded-full mb-3">
            <CheckCircle2 size={12} />Registro exitoso
          </span>
          <h2 className="text-xl font-bold text-[#0F1523]">
            {type === "entrada" ? "Entrada registrada" : "Salida registrada"}
          </h2>
          <p className="text-[#64748B] text-sm mt-1">Tu asistencia ha sido confirmada</p>
          <div className="w-full mt-6 bg-[#F8FAFC] rounded-[16px] p-4 space-y-3">
            {[
              { icon: <Clock size={15} />, label: "Hora registrada", value: time },
              { icon: type === "entrada" ? <LogIn size={15} /> : <LogOut size={15} />, label: "Tipo de registro", value: type === "entrada" ? "Entrada" : "Salida" },
              { icon: <MapPin size={15} />, label: "Ubicación", value: "Área autorizada ✓", color: "text-[#16A34A]" },
              { icon: <Shield size={15} />, label: "Empleado", value: EMPLOYEE.code },
            ].map(({ icon, label, value, color }) => (
              <div key={label} className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-[#64748B]">{icon}<span className="text-xs">{label}</span></div>
                <span className={`text-xs font-semibold ${color ?? "text-[#0F1523]"}`}>{value}</span>
              </div>
            ))}
          </div>
          <button onClick={onClose} className="w-full mt-5 h-[52px] rounded-[16px] bg-[#2563EB] text-white font-semibold text-[15px] flex items-center justify-center gap-2 shadow-[0_4px_16px_rgba(37,99,235,0.3)] hover:bg-[#1D4ED8] transition-colors">
...
function ScreenSelector({ current, onChange }: { current: AppScreen; onChange: (s: AppScreen) => void }) {
...
function AttendanceScreen({ onLogout }: { onLogout: () => void }) {
  ...
  return (
    <div className="relative w-full max-w-[430px] min-h-screen flex flex-col bg-[#F5F7FA] overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto overscroll-contain space-y-3 pb-4 pt-1" style={{ scrollbarWidth: "none" }}>
        <EmployeeCard />
        <LocationCard screen={screen} precision={precision} />
        <ClockCard now={now} />
        <MainButton screen={screen} attendanceType={attendanceType} onPress={handleRegister} />
        <InfoCard />
        {screen !== "loading" && (
          <div className="mx-4 flex items-center gap-2 flex-wrap">
            <span className={`inline-flex items-center gap-1.5 text-[11px] font-semibold px-3 py-1 rounded-full ${
              screen === "valid" || screen === "confirmed" ? "bg-[#DCFCE7] text-[#15803D]" : "bg-[#FEE2E2] text-[#B91C1C]"}`}>
              {screen === "valid" || screen === "confirmed" ? <CheckCircle2 size={11} /> : <AlertTriangle size={11} />}
              {screen === "valid" || screen === "confirmed" ? "Área permitida" : "Fuera del área"}
            </span>
      ...
      {showModal && <ConfirmationModal type={attendanceType} time={confirmedTime} onClose={handleModalClose} />}
    </div>
  );
}
    ...
    <div style={{ fontFamily: "'Inter', system-ui, sans-serif" }} className="min-h-screen">
      {view === "login" ? (
        ...
        <div className="min-h-screen bg-[#F5F7FA] flex items-start justify-center">
          <AttendanceScreen onLogout={() => setView("login")} />
        </div>
      )}
